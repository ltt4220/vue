<template>
    <div class="zm-dialog-pageBox mCustomScrollbar" theme="minimal">
        <ul class="zm-dialog-pageList">
                <li v-for="(item,i) in list" :key="i" class="zm-dialog-pageList-one sliderDragLi clearFloat" :data-sortid="item.fId" :data-show="true" :data-time="item.fCreateTime" :data-title="item.fTitle" v-bind:class="{'onlay':i==isOnlay,'onlayclick':i===isOnlayClick}" @mouseenter.stop="addEnter(i)" @mouseleave="addLeave(i)" @click.stop="addClaClick(i)">
        ...
        <div class="zm-dialog-carousel-pageSetBox" v-show="isSelect" :style="{top:top,width:pageSetBox,left:pageSetBoxLeft}">
            <div>重命名</div>
            <div>移除</div>
        </div>
    </div>
...
export default {
  ...
  methods:{
      ...
      addLeave:function(i){
            ...
            // let x=TargeAttr.x + TargeAttr.width + parseInt(_this.pageSetBox) + parseInt(_this.pageSetBoxLeft);
            // console.log(x)
            // if(PageX > x){
