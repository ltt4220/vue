  ...
  methods:{
      ...
      addLeave:function(i){
            ...
            if(event.currentTarget.className.indexOf("onlay")>-1){
                ...
                // TargeAttr.y = ;
                console.log(TargeAttr)
            }else{ return;}
            
            _this.isOnlay=-1; 
            let k = i;
            //  console.log(event.currentTarget.getBoundingClientRect().y)
            // console.log(event.currentTarget.getBoundingClientRect().top)
            console.log(k)
            document.onmousemove= function(event,k){
                event.stopPropagation();
                console.log(i)
                let PageX = event.pageX;
                let PageY = event.pageY;
                let x = TargeAttr.x + TargeAttr.width + 20 + parseInt(_this.pageSetBox);
                let y = 19 + k*TargeAttr.height; 
                // console.log(y)
                console.log(TargeAttr.height)
                if (PageX > x){
