<template>
    <div class="zm-dialog-pageBox mCustomScrollbar" theme="minimal">
        <ul class="zm-dialog-pageList">
                <li v-for="(item,i) in list" :key="i" class="zm-dialog-pageList-one sliderDragLi clearFloat" :data-sortid="item.fId" :data-show="true" :data-time="item.fCreateTime" :data-title="item.fTitle" v-bind:class="{'onlay':i==isOnlay,'onlayclick':i===isOnlayClick}" @mouseenter.stop="addEnter(i)" @mouseleave.passive="addLeave(i)" @click.stop="addClaClick(i)">
  ...
  methods:{
      ...
      addEnter:function(i){
          let _this = this;
            _this.isOnlay= i;
            var TargeAttr=null;
            if(event.currentTarget.className.indexOf("onlay")>-1){
                console.log(event.currentTarget.className)
                TargeAttr = event.currentTarget.getBoundingClientRect();
                TargeAttr.y = 19 + i * TargeAttr.height;
                console.log(TargeAttr,'------------',TargeAttr.y)
            }else{ return;}
