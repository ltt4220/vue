<template>
    <div class="zm-dialog-pageBox mCustomScrollbar" theme="minimal">
        ...
        <div class="zm-dialog-carousel-pageSetBox" @mouseenter="pageSetEnter()" @mouseleave="pageSetLeavev-show="isSelect" :style="{top:top}">
            <div>重命名</div>
            <div>移除</div>
        </div>
    </div>
...
export default {
  ...
  methods:{
      ...
      addEnter:function(i){
            let _this = this;
            _this.isOnlay= i;
            const _TargeName = event.currentTarget;
            var onlay = _TargeName.className;
            let TargeAttr = _TargeName.getBoundingClientRect();
            _this.liAttr.liAttrW = TargeAttr.width + TargeAttr.x;
            _this.liAttr.liAttrH = TargeAttr.height + TargeAttr.y;
            _this.liAttr.liAttrX = TargeAttr.x;
            _this.liAttr.liAttrY = TargeAttr.y;
            console.log(TargeAttr.y)
      },
      addLeave:function(i){
            let _this = this;
             _this.isOnlay=-1; 
