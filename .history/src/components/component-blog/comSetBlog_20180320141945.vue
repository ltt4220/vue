    ...
    <div class="zm-dialog-pageBox mCustomScrollbar" theme="minimal">
        <ul class="zm-dialog-pageList">
                <li v-for="(item,i) in list" :key="i" class="zm-dialog-pageList-one sliderDragLi clearFloat" :data-sortid="item.fId" :data-show="true" :data-time="item.fCreateTime" :data-title="item.fTitle" v-bind:class="{'onlay':i==isOnlay,'onlayclick':i===isOnlayClick}" @mouseenter.passive="addEnter(i)" @mouseleave.passive="addLeave(i)" @click.stop="addClaClick(i)">
                    ...
                    <span class="zm-dialog-eye iconfont" v-bind:class="{'icon-eyeOn': item.isIconEyeOn,'showKey': item.hasError,'icon-eyeOff':item.isIconEyeOn==false?isIconEyeOff:(i==isKey) && item.isEyeOff,'icon-cha2':item.isIconCha}" @click.stop="addHidShow(i)"></span>
                    <span class="zm-dialog-setting iconfont" v-bind:class="{'icon-shezhi2':  iconShezhi2,'setKey':item.hasError,'icon-duigou1':item.isIconDuigou1}" @click.stop="addSetting(i)"></span>
                </li>
        </ul>
...
export default {
  ...
  methods:{
      ...
      addReName:function(){
            ...
            let num = _this.num;
            for(let i = 0; i < _this.list.length; i++){
                    _this.list[i].iconShezhi2 = false;
                    _this.hasError = false;
                    _this.isIconEyeOff = false;
                    _this.list[i].isIconEyeOn = false;
                    _this.list[i].isEyeOff = false;
                    _this.isIconCha = true;
            _this.isIconDuigou1 =true;
            }
      }
  }
}
